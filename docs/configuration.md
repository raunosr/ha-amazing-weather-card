...
| `show_header`      | `true`                      | Show the title row (name, home icon, date and time). Set to `false` to remove the row; the configured name is retained. In a fixed-height grid, the chart uses the freed space. |
